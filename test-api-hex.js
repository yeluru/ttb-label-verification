...
fetch('http://localhost:3000/api/verify', { method: 'POST', body: formData })
  .then(res => res.json())
  .then(data => {
    const p = data.fields.find(f => f.fieldKey === 'producerName');
    console.log("Extracted:", p.extractedValue);
    for (let i=0; i<p.extractedValue.length; i++) console.log(p.extractedValue.charCodeAt(i));
    console.log("Submitted:", p.submittedValue);
    for (let i=0; i<p.submittedValue.length; i++) console.log(p.submittedValue.charCodeAt(i));
  });
